...
- (void)viewDidLoad {
    ...
    //_plainData = [NSData dataWithBytes:"12345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890" length:200];
...
- (IBAction)actionSecX509Enc:(id)sender{
    ...
    SecKeyRef pubkey = [QRSecCrypto RSASecKeyCreatePublicWithX509CertData:certDERData];
    if (pubkey) {
...
- (IBAction)actionSecP12Dec:(id)sender{
    ...
    SecKeyRef prikey = [QRSecCrypto RSASecKeyCreatePrivateWithP12Data:p12Data password:@"12345"];
    if (prikey) {
...
//Use +[QRFormatConvert RSA_PUB_PKCS1FromDER:] to get pkcs1Data form DER.
...
- (IBAction)actionSecKeyChainPKCS1Enc:(id)sender{
    ...
    //NSData *pkcs1Data = [QRFormatConvert RSA_PUB_PKCS1FromDER:derDataPub];
    ...
    SecKeyRef pubkey = [QRSecCrypto RSASecKeyCreatePublicWithPKCS1Data:pkcs1Data appTag:@"TestRSA"];
    if (pubkey) {
        _encryptData = [_plainData RSAEncryptDataWithPublicKey:pubkey];
        NSLog(@"[ENC]%@",_encryptData);
        CFRelease(pubkey);
...
- (IBAction)actionSecKeyChainPKCS1Dec:(id)sender{
    //Private key DER data format is same as PKCS1 format.
    ...
    SecKeyRef prikey = [QRSecCrypto RSASecKeyCreatePrivateWithDERData:derDataPri appTag:@"TestRSA"];
    if (prikey) {
        NSData *decryptData = [_encryptData RSADecryptDataWithPrivateKey:prikey];
        NSLog(@"[DEC]%@",decryptData);
        CFRelease(prikey);
...
- (IBAction)actionSeciOS10Enc:(id)sender{
    ...
    SecKeyRef pubkey = [QRSecCrypto RSASecKeyCreateWithDERData_iOS10:derDataPub isPublic:YES];
    if (pubkey) {
        _encryptData = [_plainData RSAEncryptDataWithPublicKey:pubkey];
        NSLog(@"[ENC]%@",_encryptData);
        CFRelease(pubkey);
...
- (IBAction)actionSeciOS10Dec:(id)sender{
    ...
    SecKeyRef prikey = [QRSecCrypto RSASecKeyCreateWithDERData_iOS10:derDataPri isPublic:NO];
    if (prikey) {
        NSData *decryptData = [_encryptData RSADecryptDataWithPrivateKey:prikey];
        NSLog(@"[DEC]%@",decryptData);
        CFRelease(prikey);
...
- (IBAction)actionSignAndVerify:(id)sender{
    ...
    if (prikey) {
        ...
        NSData *rawData = [NSData dataWithBytes:"Hello!" length:6];
        NSLog(@"[TXT]%@", [[NSString alloc] initWithData:rawData encoding:NSUTF8StringEncoding]);
        
        NSData *signData = [rawData RSASignDataWithPrivateKey:prikey];
        NSLog(@"[SGN]%@",signData);
        
        //Sign data can be decrypt
        //NSData *decData = [signData OpenSSL_RSA_DecryptDataWithDER:derDataPub isPublic:YES];
        ...
        if (signData) {
            ...
            if (pubkey) {
                BOOL bl = [signData RSAVerifyWithRawData:rawData publicKey:pubkey];
                NSLog(@"[Verify]%d",bl);
                CFRelease(pubkey);
            }
        }
...
//PEM <-> DER
- (IBAction)actionFormatPEMAndDER:(id)sender{
...
- (IBAction)actionOpenSSLFormatConvert:(id)sender{
    ...
    //Pri
    NSString *derPathPri = [[NSBundle mainBundle] pathForResource:@"rsa_private_key.der" ofType:nil];
    NSData *derDataPri1 = [NSData dataWithContentsOfFile:derPathPri];
    ...
    NSData *derDataPri2 = [QRFormatConvert RSA_PRI_DERFromModulus:[hexModulus dataFromHexString]
    ...
    if ([derDataPri1 isEqualToData:derDataPri2]) {
        NSLog(@"PEM is Equal!");
    }
    NSData *decryptData = [_encryptData OpenSSL_RSA_DecryptDataWithDER:derDataPri2 isPublic:NO];
    NSLog(@"[DEC]%@",decryptData);
}
